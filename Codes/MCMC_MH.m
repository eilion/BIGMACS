function [Samples,QQ] = MCMC_MH(data,Samples,core_param,global_param,stack,cal_curve,mode,nsamples)

max_iters = global_param.max_iters;
...
for iters = 1:max_iters
    ...
    LL_new(A_new>=A(2,:)|A_new<stack(1,1)) = -inf;
    
    TT_old = (A(2,:)-A_old)./(RR.*depth_diff(1));
    TT_new = (A(2,:)-A_new)./(RR.*depth_diff(1));
    
    LL_old(TT_old>0) = LL_old(TT_old>0) + (alpha-1)*log(TT_old(TT_old>0)) - beta*TT_old(TT_old>0) - log(RR(TT_old>0));
    LL_new(TT_new>0) = LL_new(TT_new>0) + (alpha-1)*log(TT_new(TT_new>0)) - beta*TT_new(TT_new>0) - log(RR(TT_new>0));
    
    LL_old(TT_old<=0) = -inf;
    LL_new(TT_new<=0) = -inf;
    
    LL_old(TT_old<core_param.lower_sedrate|TT_old>core_param.upper_sedrate) = -inf;
    LL_new(TT_new<core_param.lower_sedrate|TT_new>core_param.upper_sedrate) = -inf;
    ...
    LL_old(Z1_old==1) = LL_old(Z1_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(Z1_old==2) = LL_old(Z1_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(Z1_old==3) = LL_old(Z1_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(Z1_new==1) = LL_new(Z1_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(Z1_new==2) = LL_new(Z1_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(Z1_new==3) = LL_new(Z1_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    if strcmp(mode,'C14') == 0 && isnan(d18O(1,1)) == 0
        d18O_mu = interp1(stack(:,1),stack(:,2),A_old);
        d18O_stdv = interp1(stack(:,1),stack(:,3),A_old);
        NN = sum(isnan(d18O(1,:))==0);
        for n = 1:NN
        ...
        d18O_mu = interp1(stack(:,1),stack(:,2),A_new);
        d18O_stdv = interp1(stack(:,1),stack(:,3),A_new);
        for n = 1:NN
            LL_d18O = (1-q)*exp(-(d18O(1,n)-d18O_mu).^2./(2*d18O_stdv.^2))./sqrt(2*pi*d18O_stdv.^2) + q/2*exp(-(d18O(1,n)-d18O_mu-d*d18O_stdv).^2./(2*d18O_stdv.^2))./sqrt(2*pi*d18O_stdv.^2) + q/2*exp(-(d18O(1,n)-d18O_mu+d*d18O_stdv).^2./(2*d18O_stdv.^2))./sqrt(2*pi*d18O_stdv.^2);
    ...
    if strcmp(mode,'d18O') == 0 && isempty(C14{1}) == 0
        for ll = 1:size(C14{1},1)
            c14_mu = interp1(cal_curve{C14{1}(ll,5)}(:,1),cal_curve{C14{1}(ll,5)}(:,2),A_old);
            c14_stdv = interp1(cal_curve{C14{1}(ll,5)}(:,1),cal_curve{C14{1}(ll,5)}(:,3),A_old);
            LL_old = LL_old - (a+0.5)*log(1+(c14_mu+C14{1}(ll,3)-C14{1}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{1}(ll,2)^2+C14{1}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{1}(ll,2)^2+C14{1}(ll,4)^2);
            
            c14_mu = interp1(cal_curve{C14{1}(ll,5)}(:,1),cal_curve{C14{1}(ll,5)}(:,2),A_new);
            c14_stdv = interp1(cal_curve{C14{1}(ll,5)}(:,1),cal_curve{C14{1}(ll,5)}(:,3),A_new);
            LL_new = LL_new - (a+0.5)*log(1+(c14_mu+C14{1}(ll,3)-C14{1}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{1}(ll,2)^2+C14{1}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{1}(ll,2)^2+C14{1}(ll,4)^2);
        end
    end
    ...
    if isnan(core_param.max) == 0
        LL_old(A_old>core_param.max) = -inf;
        LL_new(A_new>core_param.max) = -inf;
    end
    if isnan(core_param.min) == 0
        LL_old(A_old<core_param.min) = -inf;
        LL_new(A_new<core_param.min) = -inf;
    end
    ...
    LL_old(Z2_old==1) = LL_old(Z2_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(Z2_old==2) = LL_old(Z2_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(Z2_old==3) = LL_old(Z2_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_old(Z1_old==1) = LL_old(Z1_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(Z1_old==2) = LL_old(Z1_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(Z1_old==3) = LL_old(Z1_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(Z2_new==1) = LL_new(Z2_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(Z2_new==2) = LL_new(Z2_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(Z2_new==3) = LL_new(Z2_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(Z1_new==1) = LL_new(Z1_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(Z1_new==2) = LL_new(Z1_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(Z1_new==3) = LL_new(Z1_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    if strcmp(mode,'C14') == 0 && isnan(d18O(2,1)) == 0
        d18O_mu = interp1(stack(:,1),stack(:,2),A_old);
        d18O_stdv = interp1(stack(:,1),stack(:,3),A_old);
        NN = sum(isnan(d18O(2,:))==0);
        for n = 1:NN
    ...
    if strcmp(mode,'d18O') == 0 && isempty(C14{2}) == 0
        for ll = 1:size(C14{2},1)
            c14_mu = interp1(cal_curve{C14{2}(ll,5)}(:,1),cal_curve{C14{2}(ll,5)}(:,2),A_old);
            c14_stdv = interp1(cal_curve{C14{2}(ll,5)}(:,1),cal_curve{C14{2}(ll,5)}(:,3),A_old);
            LL_old = LL_old - (a+0.5)*log(1+(c14_mu+C14{2}(ll,3)-C14{2}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{2}(ll,2)^2+C14{2}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{2}(ll,2)^2+C14{2}(ll,4)^2);
            
            c14_mu = interp1(cal_curve{C14{2}(ll,5)}(:,1),cal_curve{C14{2}(ll,5)}(:,2),A_new);
            c14_stdv = interp1(cal_curve{C14{2}(ll,5)}(:,1),cal_curve{C14{2}(ll,5)}(:,3),A_new);
            LL_new = LL_new - (a+0.5)*log(1+(c14_mu+C14{2}(ll,3)-C14{2}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{2}(ll,2)^2+C14{2}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{2}(ll,2)^2+C14{2}(ll,4)^2);
        end
    end
    
    if isnan(Age_Info(2,1)) == 0
        if Age_Info(2,3) == 0
    ...
    LL_old(Z_old==1) = LL_old(Z_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(Z_old==2) = LL_old(Z_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(Z_old==3) = LL_old(Z_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_old(Z_prev_old==1) = LL_old(Z_prev_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(Z_prev_old==2) = LL_old(Z_prev_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(Z_prev_old==3) = LL_old(Z_prev_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(Z_new==1) = LL_new(Z_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(Z_new==2) = LL_new(Z_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(Z_new==3) = LL_new(Z_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(Z_prev_new==1) = LL_new(Z_prev_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(Z_prev_new==2) = LL_new(Z_prev_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(Z_prev_new==3) = LL_new(Z_prev_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
            ...
            if index_0(n) == 1
                ...
                if isempty(C14{n}) == 0
                    for ll = 1:size(C14{n},1)
                        c14_mu = interp1(cal_curve{C14{n}(ll,5)}(:,1),cal_curve{C14{n}(ll,5)}(:,2),A_old(m,:));
                        c14_stdv = interp1(cal_curve{C14{n}(ll,5)}(:,1),cal_curve{C14{n}(ll,5)}(:,3),A_old(m,:));
                        LL_C14_old(m,:) = LL_C14_old(m,:) - (a+0.5)*log(1+(c14_mu+C14{n}(ll,3)-C14{n}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{n}(ll,2)^2+C14{n}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{n}(ll,2)^2+C14{n}(ll,4)^2);
                        
                        c14_mu = interp1(cal_curve{C14{n}(ll,5)}(:,1),cal_curve{C14{n}(ll,5)}(:,2),A_new(m,:));
                        c14_stdv = interp1(cal_curve{C14{n}(ll,5)}(:,1),cal_curve{C14{n}(ll,5)}(:,3),A_new(m,:));
                        LL_C14_new(m,:) = LL_C14_new(m,:) - (a+0.5)*log(1+(c14_mu+C14{n}(ll,3)-C14{n}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{n}(ll,2)^2+C14{n}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{n}(ll,2)^2+C14{n}(ll,4)^2);
                    end
                end
            end
        end
    ...
    LL_new(A_new<=A(N-1,:)|A_new>stack(end,1)) = -inf;
    ...
    LL_old(ZN1_old==1) = LL_old(ZN1_old==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_old(ZN1_old==2) = LL_old(ZN1_old==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_old(ZN1_old==3) = LL_old(ZN1_old==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    LL_new(ZN1_new==1) = LL_new(ZN1_new==1) - log(gamcdf(0.9220,alpha,1/beta));
    LL_new(ZN1_new==2) = LL_new(ZN1_new==2) - log(gamcdf(1.0850,alpha,1/beta)-gamcdf(0.9220,alpha,1/beta));
    LL_new(ZN1_new==3) = LL_new(ZN1_new==3) - log(1-gamcdf(1.0850,alpha,1/beta));
    
    if strcmp(mode,'C14') == 0 && isnan(d18O(N,1)) == 0
        d18O_mu = interp1(stack(:,1),stack(:,2),A_old);
        d18O_stdv = interp1(stack(:,1),stack(:,3),A_old);
        NN = sum(isnan(d18O(N,:))==0);
        for n = 1:NN
    ...
    if strcmp(mode,'d18O') == 0 && isempty(C14{N}) == 0
        for ll = 1:size(C14{N},1)
            c14_mu = interp1(cal_curve{C14{N}(ll,5)}(:,1),cal_curve{C14{N}(ll,5)}(:,2),A_old);
            c14_stdv = interp1(cal_curve{C14{N}(ll,5)}(:,1),cal_curve{C14{N}(ll,5)}(:,3),A_old);
            LL_old = LL_old - (a+0.5)*log(1+(c14_mu+C14{N}(ll,3)-C14{N}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{N}(ll,2)^2+C14{N}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{N}(ll,2)^2+C14{N}(ll,4)^2);
            
            c14_mu = interp1(cal_curve{C14{N}(ll,5)}(:,1),cal_curve{C14{N}(ll,5)}(:,2),A_new);
            c14_stdv = interp1(cal_curve{C14{N}(ll,5)}(:,1),cal_curve{C14{N}(ll,5)}(:,3),A_new);
            LL_new = LL_new - (a+0.5)*log(1+(c14_mu+C14{N}(ll,3)-C14{N}(ll,1)).^2./(2*b*(c14_stdv.^2+C14{N}(ll,2)^2+C14{N}(ll,4)^2))) - 0.5*log(c14_stdv.^2+C14{N}(ll,2)^2+C14{N}(ll,4)^2);
        end
    end
    
    if isnan(Age_Info(N,1)) == 0
        if Age_Info(N,3) == 0
...
if strcmp(mode,'C14') == 0
    for m = 1:M
        ...
        mu = interp1(stack(:,1),stack(:,2),A(:,m));
        sig = interp1(stack(:,1),stack(:,3),A(:,m));
