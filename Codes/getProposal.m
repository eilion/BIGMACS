function [QQ] = getProposal(data,core_param,global_param,stack,cal_curve,mode)
...
% initialization:
[A{N},W{N}] = Proposal_3([],[],[],d18O(N,:),C14{N},Age_Info(N,:),core_param,global_param,S,stack,cal_curve,mode);

% iteration:
for nn = 1:N-1
    n = N-nn;
    [A{n},W{n}] = Proposal_3(W{n+1},A{n+1},depth_diff(n),d18O(n,:),C14{n},Age_Info(n,:),core_param,global_param,S,stack,cal_curve,mode);
end
